 ...
 * @link https://github.com/WebDevStudios/CMB2-Snippet-Library/blob/master/front-end/cmb2-front-end-submit.php Snippet
 */
 ...
 */
function bookcpt_frontend_form_register() {
	...
	$cmb->add_field( array(
		'name'    => __( 'New Book Title', 'wpsessions' ),
		'id'      => 'submitted_post_title',
		'type'    => 'text',
		'default' => __( 'New Book', 'wpsessions' ),
	) );

	$cmb->add_field( array(
		'name'    => __( 'New Book Content', 'wpsessions' ),
	...
	$cmb->add_field( array(
		'name'       => __( 'Featured Image for New Book', 'wpsessions' ),
	...
	$cmb->add_field( array(
		'name'      => __( '(Optionally) Select Related Books', 'books-cpt' ),
		'id'        => $prefix . 'related',
	...
	$cmb->add_field( array(
		'name' => __( 'Your Name', 'wpsessions' ),
		'desc' => __( 'Please enter your name for author credit on the book submission.', 'wpsessions' ),
		'id'   => $prefix . 'submitter',
		'type' => 'text',
	) );

	$cmb->add_field( array(
		'name' => __( 'Your URL', 'wpsessions' ),
		'id'   => $prefix . 'submitter_url',
		'type' => 'text_url',
	) );

}
add_action( 'cmb2_init', 'bookcpt_frontend_form_register' );
 ...
 * @return CMB2 object
 */
function bookcpt_frontend_cmb2_get() {
 ...
 */
function bookcpt_do_frontend_form_submission_shortcode( $atts = array() ) {

	// Get CMB2 metabox object
	$cmb = bookcpt_frontend_cmb2_get();
	...
	if ( isset( $_GET['post_submitted'] ) && ( $post = get_post( absint( $_GET['post_submitted'] ) ) ) ) {
		...
		$name = get_post_meta( $post->ID, '_bookcpt_submitter', 1 );
		$name = $name ? ' '. $name : '';

		// Add notice of submission to our output
		$output .= '<h3>' . sprintf( __( 'Thank you%s, your book has been submitted and is pending review by a site administrator.', 'wpsessions' ), esc_html( $name ) ) . '</h3>';
	}

	// Get our form
	$output .= cmb2_get_metabox_form( $cmb, 'fake-oject-id', array( 'save_button' => __( 'Submit Book', 'wpsessions' ) ) );

	return $output;
}
add_shortcode( 'book-submission', 'bookcpt_do_frontend_form_submission_shortcode' );
 ...
 * @return void
 */
function bookcpt_handle_frontend_new_post_form_submission() {
	...
	$img_id = bookcpt_frontend_form_photo_upload( $new_submission_id, $post_data );
	...
	if ( ! empty( $_POST['_bookcpt_genres'] ) ) {
		$terms = array();
		foreach ( (array) $_POST['_bookcpt_genres'] as $genre_slug ) {
			if ( $term = get_term_by( 'slug', $genre_slug, 'genre' ) ) {
				$terms[] = $term->term_id;
			}
		}
		if ( ! empty( $terms ) ) {
			wp_set_post_terms( $new_submission_id, $terms, 'genre' );
		}
	}
...
add_action( 'cmb2_after_init', 'bookcpt_handle_frontend_new_post_form_submission' );
 ...
 */
function bookcpt_frontend_form_photo_upload( $post_id, $attachment_post_data = array() ) {
